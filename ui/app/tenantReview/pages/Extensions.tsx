...
export const Extensions: React.FC = () => {
  const result = useExtensionsReview();

  if (result.isLoading) {
    return <LoadingState message="Analyzing extensions..." />;
  }
    ...
    <Flex flexDirection="column" gap={20}>
      <Flex justifyContent="space-between" alignItems="flex-start">
        <Flex flexDirection="column" gap={8}>
          ...
          <Text>Extensions 1.0 vs 2.0 migration status and extension health.</Text>
        </Flex>
        <Flex gap={12} alignItems="center">
          <ExportButtons title="Extensions" findings={result.findings} summary={[`Score: ${result.score.value}/100`, `Migration: ${result.migration.percentage}% Gen3`, `${result.score.passedChecks}/${result.score.totalChecks} checks passed`]} />
