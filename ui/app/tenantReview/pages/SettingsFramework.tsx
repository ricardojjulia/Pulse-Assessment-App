...
export const SettingsFramework: React.FC = () => {
  const result = useSettingsReview();

  if (result.isLoading) {
    return <LoadingState message="Analyzing Settings 2.0 configuration..." />;
  }
    ...
    <Flex flexDirection="column" gap={20}>
      <Flex justifyContent="space-between" alignItems="flex-start">
        <Flex flexDirection="column" gap={8}>
          <Heading level={1}>Settings Framework</Heading>
          <Text>Settings v1 vs Settings 2.0 adoption and configuration migration status.</Text>
        </Flex>
        <Flex gap={12} alignItems="center">
          <ExportButtons title="Settings Framework" findings={result.findings} summary={[`Score: ${result.score.value}/100`, `Migration: ${result.migration.percentage}% Gen3`, `${result.score.passedChecks}/${result.score.totalChecks} checks passed`]} />
