...
export const ApiAccess: React.FC = () => {
  const result = useApiAccessReview();

  if (result.isLoading) return <LoadingState message="Analyzing API and access management..." />;
    ...
    <Flex flexDirection="column" gap={20}>
      <Flex justifyContent="space-between" alignItems="flex-start">
        <Flex flexDirection="column" gap={8}>
          <Heading level={1}>API & Access Management</Heading>
          <Text>API token inventory, OAuth adoption, and credential vault management.</Text>
        </Flex>
        <Flex gap={12} alignItems="center">
          <ExportButtons title="API & Access Management" findings={result.findings} summary={[`Score: ${result.score.value}/100`, `Migration: ${result.migration.percentage}% Gen3`, `${result.score.passedChecks}/${result.score.totalChecks} checks passed`]} />
