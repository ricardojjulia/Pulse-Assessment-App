...
export const Metrics: React.FC = () => {
  const result = useMetricsReview();

  if (result.isLoading) return <LoadingState message="Analyzing metrics configuration..." />;
    ...
    <Flex flexDirection="column" gap={20}>
      <Flex justifyContent="space-between" alignItems="flex-start">
        <Flex flexDirection="column" gap={8}>
          ...
          <Text>Custom metrics, calculated metrics migration, and metric event status.</Text>
        </Flex>
        <Flex gap={12} alignItems="center">
          <ExportButtons title="Metrics" findings={result.findings} summary={[`Score: ${result.score.value}/100`, `Migration: ${result.migration.percentage}% Gen3`, `${result.score.passedChecks}/${result.score.totalChecks} checks passed`]} />
