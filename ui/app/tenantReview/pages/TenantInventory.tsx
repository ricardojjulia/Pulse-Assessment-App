...
const SECTION_META: Record<string, SectionMeta> = {
  ...
  "inv-storage": {
    ...
    metrics: [
      ...
      { item: "Events in Grail (7d)", label: "Events in Grail (7d)" },
      { item: "Business Events (24h)", label: "Business Events (24h)" },
      { item: "OTel Spans (24h)", label: "OTel Spans (24h)" },
      { item: "DPS Full-Stack Usage (GiB, recent)", label: "DPS Full-Stack" },
    ],
    barItems: ["Log Volume (24h)", "Business Events (24h)"],
  },
  "inv-synthetic": {
    ...
    metrics: [
      ...
      { item: "Grail Synthetic Executions (7d)", label: "Executions (7d)" },
    ],
    barItems: ["Grail Synthetic Executions (7d)"],
  ...
  "inv-alerting": {
    ...
    metrics: [
      ...
      { item: "Davis Problems (7d)", label: "Problems (7d)" },
      { item: "Davis Events in Grail (7d)", label: "Davis Events (7d)" },
    ],
    barItems: ["Davis Problems (7d)"],
    ...
    async function collect() {
      ...
      result.push({
        ...
        rows: [
          ...
          { item: "Davis Problems (7d)", value: num(dqlCount(davisProblems)), detail: dqlRecords(problemsByStatus).map((r) => `${String(r["event.status"])}: ${num(Number(r["count()"] ?? 0))}`).join(", ") || undefined },
          { item: "Davis Events in Grail (7d)", value: num(dqlCount(davisEvents)) },
      ...
      result.push({
        ...
        rows: [
          ...
          { item: "Events in Grail (7d)", value: num(dqlCount(eventCount)) },
          { item: "Business Events (24h)", value: num(dqlCount(bizEvents)), detail: dqlRecords(bizEventTypes).map((r) => `${String(r["event.type"])}: ${num(Number(r["eventCount"] ?? r["count()"] ?? 0))}`).join(", ") || undefined },
          { item: "OTel Spans (24h)", value: spans.error ? "N/A (needs storage:spans:read scope)" : num(dqlCount(spans)) },
      ...
      result.push({
        ...
        rows: [
          ...
          { item: "Grail Synthetic Executions (7d)", value: num(dqlCount(grailSynthetic)), detail: dqlRecords(grailSynTypes).map((r) => `${String(r["event.type"])}: ${num(Number(r["eventCount"] ?? r["count()"] ?? 0))}`).join(", ") || undefined },
