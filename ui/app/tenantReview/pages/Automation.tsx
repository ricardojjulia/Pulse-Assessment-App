...
export const Automation: React.FC = () => {
  const result = useAutomationReview();

  if (result.isLoading) return <LoadingState message="Analyzing automation and workflows..." />;
    ...
    <Flex flexDirection="column" gap={20}>
      <Flex justifyContent="space-between" alignItems="flex-start">
        <Flex flexDirection="column" gap={8}>
          <Heading level={1}>Automation & Workflows</Heading>
          <Text>Workflow adoption, classic notification integrations, and automation rules.</Text>
        </Flex>
        <Flex gap={12} alignItems="center">
          <ExportButtons title="Automation & Workflows" findings={result.findings} summary={[`Score: ${result.score.value}/100`, `Migration: ${result.migration.percentage}% Gen3`, `${result.score.passedChecks}/${result.score.totalChecks} checks passed`]} />
