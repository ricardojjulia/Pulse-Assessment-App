...
export const RumSession: React.FC = () => {
  const result = useRumReview();

  if (result.isLoading) return <LoadingState message="Analyzing RUM and session replay..." />;
    ...
    <Flex flexDirection="column" gap={20}>
      <Flex justifyContent="space-between" alignItems="flex-start">
        <Flex flexDirection="column" gap={8}>
          <Heading level={1}>RUM & Session Replay</Heading>
          <Text>Real User Monitoring configuration, session replay, and USQL to DQL migration.</Text>
        </Flex>
        <Flex gap={12} alignItems="center">
          <ExportButtons title="RUM & Session Replay" findings={result.findings} summary={[`Score: ${result.score.value}/100`, `Migration: ${result.migration.percentage}% Gen3`, `${result.score.passedChecks}/${result.score.totalChecks} checks passed`]} />
