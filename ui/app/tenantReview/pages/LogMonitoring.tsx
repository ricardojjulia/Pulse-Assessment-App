...
export const LogMonitoring: React.FC = () => {
  const result = useLogMonitoringReview();

  if (result.isLoading) return <LoadingState message="Analyzing log monitoring..." />;
    ...
    <Flex flexDirection="column" gap={20}>
      <Flex justifyContent="space-between" alignItems="flex-start">
        <Flex flexDirection="column" gap={8}>
          <Heading level={1}>Log Monitoring</Heading>
          <Text>Classic vs Grail log ingestion, OpenPipeline configuration, and log processing rules.</Text>
        </Flex>
        <Flex gap={12} alignItems="center">
          <ExportButtons title="Log Monitoring" findings={result.findings} summary={[`Score: ${result.score.value}/100`, `Migration: ${result.migration.percentage}% Gen3`, `${result.score.passedChecks}/${result.score.totalChecks} checks passed`]} />
