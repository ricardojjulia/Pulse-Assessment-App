...
export const Synthetic: React.FC = () => {
  const result = useSyntheticReview();

  if (result.isLoading) return <LoadingState message="Analyzing synthetic monitoring..." />;
    ...
    <Flex flexDirection="column" gap={20}>
      <Flex justifyContent="space-between" alignItems="flex-start">
        <Flex flexDirection="column" gap={8}>
          <Heading level={1}>Synthetic Monitoring</Heading>
          <Text>Browser and HTTP monitors, locations, and execution health.</Text>
        </Flex>
        <Flex gap={12} alignItems="center">
          <ExportButtons title="Synthetic Monitoring" findings={result.findings} summary={[`Score: ${result.score.value}/100`, `Migration: ${result.migration.percentage}% Gen3`, `${result.score.passedChecks}/${result.score.totalChecks} checks passed`]} />
