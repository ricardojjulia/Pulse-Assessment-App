...
export const Security: React.FC = () => {
  const result = useSecurityReview();

  if (result.isLoading) return <LoadingState message="Analyzing security posture..." />;
    ...
    <Flex flexDirection="column" gap={20}>
      <Flex justifyContent="space-between" alignItems="flex-start">
        <Flex flexDirection="column" gap={8}>
          ...
          <Text>Application security, runtime vulnerability analytics, and attack detection.</Text>
        </Flex>
        <Flex gap={12} alignItems="center">
          <ExportButtons title="Security" findings={result.findings} summary={[`Score: ${result.score.value}/100`, `Migration: ${result.migration.percentage}% Gen3`, `${result.score.passedChecks}/${result.score.totalChecks} checks passed`]} />
