...
export const MonitoringConfig: React.FC = () => {
  const result = useMonitoringReview();

  if (result.isLoading) return <LoadingState message="Analyzing monitoring configuration..." />;
    ...
    <Flex flexDirection="column" gap={20}>
      <Flex justifyContent="space-between" alignItems="flex-start">
        <Flex flexDirection="column" gap={8}>
          <Heading level={1}>Monitoring Configuration</Heading>
          <Text>OneAgent deployment, ActiveGate status, host groups, and monitoring modes.</Text>
        </Flex>
        <Flex gap={12} alignItems="center">
          <ExportButtons title="Monitoring Configuration" findings={result.findings} summary={[`Score: ${result.score.value}/100`, `Migration: ${result.migration.percentage}% Gen3`, `${result.score.passedChecks}/${result.score.totalChecks} checks passed`]} />
